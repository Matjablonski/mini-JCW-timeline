  ...
  (context) => {
    ...
    function setImagesRotation() {
      imgContainers.forEach((container) => {
        ...
        wrappers.forEach((wrap, index) => {
          wrap.style.transform = `translate(-50%, 0%) rotate(${
            calDeg * index
          }deg)`
        })
      })
      gsap.set(imgContainers[1], { scale: isDesktop ? 0.62 : 0.64 })
    }
    ...
    function timelineEntrance() {
      ...
      const horizontalScroll = gsap.to(sections, {
        xPercent: !isMobile ? -100 * sections.length : 0,
        ease: 'none',
        scrollTrigger: {
      ...
      const createSectionTimeline = (section, yPercent) => {
        const timeline = gsap.timeline({
          scrollTrigger: {
            ...
            containerAnimation: !isMobile ? horizontalScroll : undefined,
            scrub: true,
          },
        })
